...
If you see a printout of the version number as shown above, this means you have Git already. Great! But wait. Is the version number < 2.28? If so, you'll need to update (follow the install instructions based on your operating system below). The reason you  need to update if you have an older version of Git is that newer versions allow us to configure the default branch name. There is a movement in the Git community to [rename the default branch of repositories from 'master' in favor of the neutral term 'main'](https://github.com/github/renaming), as 'master' has [offensive connotations](https://www.bbc.com/news/technology-53050955).

If you see something more like `git: command not found`, proceed to the instructions for installation based on your operating system.

Otherwise, if you have a current version of Git installed, you can skip this step!
...
After running this, check the version is now updated. Additionally, save yourself some headache and [make sure that RStudio is pointing to the correct version of git](https://happygitwithr.com/rstudio-see-git.html).

If you need to install Git anew, first install the Xcode command line tools (not all of Xcode), which includes Git by opening a terminal and typing in the following and press enter. Follow prompts. 
...
Take note of the "Code" green button on the upper right corner of the file list. This is where you can obtain the URL to this repository if someone else were to clone your repository. Keep in mind that the owner of this repository must first add the individuals to the collaborators list in order for them to have access, since this repo is private to only you and the teaching team. This is done by going to 'Settings' - see below.
